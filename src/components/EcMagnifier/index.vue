...
const emits = defineEmits(['mousemove', 'mouseleave', 'mouseenter', 'created'])
...
const imgInfo = ref({})
const $img = ref(null)
const beforeReactivateMoveFns = ref([])
...
const addResizeListener = (dom, cb) => {
    ...
    if (!imgLoadedFlag.value) {
      ...
      if ($img) {
        ...
        setTimeout(() => {
          ...
          handlerImgResize()
          emits('created', $img, imgInfo.value)
        }, 500)
      }
    }
  })
...
const validImgResize = (newImgInfo) => {
...
const mouseMove = (e) => {
  ...
  if (imgLoadedFlag.value && e) {
    beforeReactivateMoveFns.value.forEach((fn) => fn.call(null))
...
onMounted(() => {
  ...
  if (imgRef.value) {
    addResizeListener(imgRef.value, (rect) => {
      imgInfo.value = rect
      handlerImgResize()
    })
  }
