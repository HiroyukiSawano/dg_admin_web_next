...
const _series = computed(() => {
  if (dataset.value) {
    ...
    if(dimensions) {
      return dimensions.slice(1).map(() => ({ type: 'bar', barCategoryGap: '50%' }))
    }
    if(isArray(source[0])) {
      return source[0].slice(1).map(() => ({ type: 'bar', barCategoryGap: '50%' }))
    }
    if(isPlainObject(source[0])) {
      return Object.keys(source[0]).slice(1).map(() => ({ type: 'bar', barCategoryGap: '50%' }))
    }
  }
